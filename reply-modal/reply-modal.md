...
![Reply Modal](reply-modal.png)

```html
<!-- todo:: consider change input id if needed  -->
		...
		<div class="modal-content" style="border-radius: 16px; border: none;">
			<div class="modal-body p-4">
				...
				<div class="d-flex align-items-center gap-3 mb-4">
					<img src="~/assets/images/person.svg" alt="صالح محمد" class="rounded-circle"
						style="width: 48px; height: 48px;" />
					<div>
						<h6 class="mb-0 fw-bold">صالح محمد</h6>
						<small class="text-muted">مستشار</small>
					</div>
				</div>

				<form id="addReplyForm" enctype="multipart/form-data">
					<!-- Reply Text Area -->
					<div class="mb-4">
						<textarea class="form-control" id="replyText" name="replyText" placeholder="اكتب تعليقك هنا..." required
							style="border: 1px solid #e0e0e0; border-radius: 12px; resize: none; min-height: 120px; padding: 16px; font-size: 14px; line-height: 1.5;"
							rows="4"></textarea>
					</div>
					...
					<div class="d-flex flex-column align-items-center w-100">
						...
						<section class="w-100 px-3">
							<div class="d-flex align-items-center gap-2 drag-area mb-2 bg-transparent">
								...
								<button type="button" class="btn file-input-button">
									إضافة مرفق
								</button>
								<input type="file" class="file-input" hidden />
								<!-- Images groups - All the selected images will be shown here -->
								<p class="text-center text-sm hidden text-danger m-2" id="filesize-error">
									حجم الملف يجب أن يكون أقل من 5 ميجا بايت
								</p>
								<p class="text-center text-sm hidden text-danger m-2" id="filetype-error">
									الملف يجب أن يكون بصيغة Docx أو PDF أو PNG أو JPG
								</p>
							</div>
						</section>
						<ul class="w-100 px-0" id="document-images"></ul>
					</div>
					...
					<div class="d-flex justify-content-between align-items-center">
						<div class="d-flex gap-3">
							<button type="button" class="btn btn-outline-secondary px-4 py-2" data-bs-dismiss="modal"
								style="border-radius: 8px; border-color: #e0e0e0; color: #6B7280;">
								إلغاء
							</button>
							<button type="button" class="btn px-4 py-2" id="saveReplyBtn"
								style="background-color: #00A79D; border: none; color: white; border-radius: 8px;">
								نشر
							</button>
						</div>
					</div>
...
<!-- handle drag and drop upload files and list them -->
