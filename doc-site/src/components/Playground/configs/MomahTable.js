export const MomahTable = {
  ...
  css: `/* this file contains data table styles and action button with dropdown */
...
// Render/Initialization logic
(function() {
  const rawCols = \`\${props.columnsRaw}\`.split(',').map(c => c.trim());
  const columns = rawCols.map((c, i) => {
    let field = 'col_' + i;
    let render = null;
    if (c === 'رقم الطلب') field = 'id';
    else if (c === 'تاريخ التقديم') field = 'date';
    else if (c === 'اسم المنشأة') field = 'company';
    else if (c === 'حالة الطلب') {
      field = 'status';
      render = (rec) => {
        if (rec.status === 'نشط' || rec.status === 'active') {
          return '<span class="status-pill status-published">نشط</span>';
        } else if (rec.status === 'معلق' || rec.status === 'pending') {
          return '<span class="status-pill status-unpublished">معلق</span>';
        }
        return rec.status;
      };
    }
    return { header: c, field: field, render: render };
  });

  const data = [
    { id: '#10928', date: '2026-07-05', company: 'شركة الأفكار الرائدة', status: 'نشط' },
    { id: '#10929', date: '2026-07-04', company: 'مؤسسة الحلول المبتكرة', status: 'معلق' }
  ];

  // For any custom column that does not match standard fields, provide mock values
  data.forEach((row, rowIndex) => {
    columns.forEach(col => {
      if (!(col.field in row)) {
        row[col.field] = 'بيانات ' + (rowIndex + 1);
      }
    });
  });

  const options = {
    search: \`\${props.searchable}\` === 'true' || \`\${props.searchable}\` === '1' || \`\${props.searchable}\` === true ? {
      containerSelector: '#tableActions',
      inputId: 'searchInput',
      placeholder: 'بحث عام...'
    } : null,
    pagination: \`\${props.pagination}\` === 'true' || \`\${props.pagination}\` === '1' || \`\${props.pagination}\` === true ? {
      ...
      onPageChange: (page) => {
        ...
        if (summary) {
          if (page === 1) {
            summary.textContent = 'عرض 1 إلى 2 من أصل 2 سجلات';
          } else {
            summary.textContent = 'عرض 3 إلى 4 من أصل 2 سجلات';
          }
        }
      }
    } : null
...
<div class="momah-table-host">
  <div style="width: 100%; text-align: right;" dir="rtl">
    ...
    <div id="pagination" style="display: flex; justify-content: space-between; align-items: center; margin-top: 1rem;">
      <div id="paginationSummary" style="font-size: 0.85rem; color: #6c757d;">عرض 1 إلى 2 من أصل 2 سجلات</div>
...
<script>
  (function() {
    ...
    const options = {
      ${props.searchable ? 'search: { containerSelector: "#tableActions", placeholder: "بحث..." },' : ''}
      ${props.pagination ? 'pagination: { containerSelector: "#paginationContainer", currentPage: 1, totalPages: 10, onPageChange: (page) => {} },' : ''}
    };
    MomahTable.render('#tableContainer', columns, data, options);
  })();
</script>`,
